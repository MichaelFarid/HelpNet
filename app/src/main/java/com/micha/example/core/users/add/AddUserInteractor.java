...
import com.micha.example.utils.Constants;
import com.micha.example.utils.SharedPrefUtil;
...
public class AddUserInteractor implements AddUserContract.Interactor {
    ...
    public void addUserToDatabase(final Context context, FirebaseUser firebaseUser, String Name,String age, String height, String weight,String gender, boolean c, boolean a,boolean e) {
        DatabaseReference database = FirebaseDatabase.getInstance().getReference();
        User user = new User(firebaseUser.getUid(),
                firebaseUser.getEmail(),
                new SharedPrefUtil(context).getString(Constants.ARG_FIREBASE_TOKEN), Name,age, height, weight,gender, c,a,e);
        database.child(Constants.ARG_USERS)
                .child(firebaseUser.getUid())
                .setValue(user)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
